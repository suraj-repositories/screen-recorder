package com.oranbyte.screenrec.constants;

import java.awt.Color;

public final class AppColors {

	private AppColors() {
	}

	public static final Color BACKGROUND = new Color(0xF5F5F5);
	public static final Color SURFACE = Color.WHITE;
	
	public static final Color WHITE = Color.WHITE;
	public static final Color CARD_BG = new Color(245, 247, 250);
 
	public static final Color PRIMARY = new Color(0xF57C00);
	public static final Color PRIMARY_HOVER = new Color(0xFB8C00);
	public static final Color PRIMARY_PRESSED = new Color(0xEF6C00);
	public static final Color DANGER_COLOR = new Color(220, 38, 38);
	public static final Color DANGER_HOVER = new Color(185, 28, 28);
	public static final Color NEUTRAL_COLOR = new Color(226, 232, 240);
	public static final Color NEUTRAL_HOVER = new Color(203, 213, 225); 

	public static final Color TEXT = new Color(0x333333);
	public static final Color TEXT_SECONDARY = new Color(0x757575);
	public static final Color TEXT_MUTED = new Color(130, 130, 138);

	public static final Color BORDER = new Color(0xD8D8D8);
	public static final Color RECORDING_BORDER_COLOR = new Color(0xff907d);
	public static final Color BORDER_HOVER = PRIMARY;

	public static final Color BUTTON = SURFACE;
	public static final Color BUTTON_HOVER = new Color(0xF7F7F7);
	public static final Color BUTTON_PRESSED = new Color(0xECECEC);

	public static final Color SELECTION_OUTLINE_COLOR = Color.WHITE;
	public static final Color SELECTION_RESIZER_COLOR = Color.WHITE;

	public static final Color GRAY_100 = new Color(245, 245, 245);
	public static final Color GRAY_200 = new Color(229, 229, 229);
	public static final Color GRAY_300 = new Color(212, 212, 212);
	public static final Color GRAY_400 = new Color(184, 184, 184);
	public static final Color GRAY_500 = new Color(115, 115, 115);
	public static final Color GRAY_600 = new Color(82, 82, 82);
	public static final Color GRAY_700 = new Color(64, 64, 64);
	
	public static final Color TRANSPARENT = new Color(0, 0, 0, 0);

}